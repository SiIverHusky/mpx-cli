"""mpx versions — list published versions of a marketplace skill."""
...
def cmd_versions(args: argparse.Namespace) -> None:
    ...
    ver_width = max(max(len(str(v.get("version", "?"))) for v in versions) + 2, 10)
    header = f"  {'Version':<{ver_width}} Published"
    print(header)
    print(f"  {'-' * (ver_width + 30)}")

    for v in sorted(versions, key=lambda x: str(x.get("created_at", x.get("published_at", "")))):
        ver = v.get("version", "?")
        date = v.get("created_at", v.get("published_at", "?"))
        print(f"  {str(ver):<{ver_width}} {date}")
    print()
